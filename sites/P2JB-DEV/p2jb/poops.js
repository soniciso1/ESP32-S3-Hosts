...
export function makePoopsEngine(X) {
  ...
  /* KIT ID PRESERVATION (devkit-only site).
   *
   * kexp's qaf patch rewrites the kernel's target_id from 0x81 (devkit) to 0x82
   * (testkit) along with security_flags/qa_flags/utoken_flags:
   *     before qaf { security_flags: 0x23 target_id: 0x81 ... }
   *     new    qaf { security_flags: 0x37 target_id: 0x82 ... }
   * On a devkit that is a visible mode change, and a devkit already has the
   * privileges the 0x82 spoof exists to obtain. kexp is a prebuilt blob shared with
   * the retail build, so it cannot be patched here - the value is snapshotted before
   * the handoff and written back afterwards instead.
   *
   * It restores the value THIS console had, rather than forcing 0x81: a retail box
   * loading this page keeps whatever it started with, and nothing is assumed about
   * which kit is running.
   *
   * target_id sits at security_flags+9 on every firmware we carry - the same +9 kexp
   * itself reports between security_flags_ptr and target_id_ptr. The two anchors below
   * are the verified DATA_BASE values from p2jb.js's table, not new constants. */
  const KITID_FW = {
    "7.00":  { allproc: 0x02859D50, secflags: 0x00AC8064 },
    "7.01":  { allproc: 0x02859D50, secflags: 0x00AC8064 },
    "7.20":  { allproc: 0x02859D50, secflags: 0x00AC8064 },
    "7.40":  { allproc: 0x02859D50, secflags: 0x00AC8064 },
    "7.60":  { allproc: 0x02859D50, secflags: 0x00AC8064 },
    "7.61":  { allproc: 0x02859D50, secflags: 0x00AC8064 },
    "8.00":  { allproc: 0x02875D50, secflags: 0x00AC3064 },
    "8.20":  { allproc: 0x02875D50, secflags: 0x00AC3064 },
    "8.40":  { allproc: 0x02875D50, secflags: 0x00AC3064 },
    "8.60":  { allproc: 0x02875D50, secflags: 0x00AC3064 },
    "9.00":  { allproc: 0x02755D50, secflags: 0x00D72064 },
    "9.05":  { allproc: 0x02755D50, secflags: 0x00D73064 },
    "9.20":  { allproc: 0x02755D50, secflags: 0x00D73064 },
    "9.40":  { allproc: 0x02755D50, secflags: 0x00D73064 },
    "9.60":  { allproc: 0x02755D50, secflags: 0x00D73064 },
    "10.00": { allproc: 0x02765D70, secflags: 0x00D79064 },
    "10.01": { allproc: 0x02765D70, secflags: 0x00D79064 },
    "10.20": { allproc: 0x02765D70, secflags: 0x00D79064 },
    "10.40": { allproc: 0x02765D70, secflags: 0x00D79064 },
    "10.60": { allproc: 0x02765D70, secflags: 0x00D79064 },
    "11.00": { allproc: 0x02875D70, secflags: 0x00D8C064 },
    "11.20": { allproc: 0x02875D70, secflags: 0x00D8C064 },
    "11.40": { allproc: 0x02875D70, secflags: 0x00D8C064 },
    "11.60": { allproc: 0x02875D70, secflags: 0x00D8C064 },
    "12.00": { allproc: 0x02885E00, secflags: 0x00D83064 },
    "12.02": { allproc: 0x02885E00, secflags: 0x00D83064 },
    "12.20": { allproc: 0x02885E00, secflags: 0x00D83064 },
    "12.40": { allproc: 0x02885E00, secflags: 0x00D83064 },
    "12.60": { allproc: 0x02885E00, secflags: 0x00D83064 },
    "12.70": { allproc: 0x02885E00, secflags: 0x00D83064 },
  };

  function kitIdAddr() {
    try {
      const fw = String(window.fw_str || "");
      const e = KITID_FW[fw];
      if (!e) { flushMark("KITID", "no-anchor-for-fw=" + fw); return null; }
      if (!S.allproc) { flushMark("KITID", "allproc-unknown"); return null; }
      /* data_base = &allproc - DATA_BASE_ALLPROC, exactly as p2jb.js derives it. */
      return S.allproc.sub32(e.allproc).add32(e.secflags).add32(9);
    } catch (e) {
      flushMark("KITID", "addr-failed-" + String((e && e.message) || e).slice(0, 50));
      return null;
    }
  }

  /* Keep this console's own target_id by patching the kexp BLOB before it runs, rather
     than writing kernel memory afterwards. This is the approach p2jb.js uses and which is
     hardware-verified on a 12.70 devkit; the earlier snapshot/restore pair is left in
     place but unused.

     In kexp's patch_qa_flags every field is a read-modify-write that ORs bits onto the
     live value -- except target_id, which is a hard overwrite:
         0x3807  c6 45 d6 82     mov byte [rbp-0x2a], 0x82   <- dropped here
         0x382a  ba 82 00 00 00  mov edx, 0x82               <- made to log the live byte
     NOPing 0x3807 leaves security_flags / qa_flags / utoken_flags exactly as kexp sets
     them, which is what the jailbreak needs; only the kit-id overwrite goes away.

     Applied for exactly ONE observed live value, 0x81. A retail console, an unknown id,
     or a failed read all take the byte-identical stock path -- keeping a RETAIL id is the
     case that panics, so it must stay unreachable. */
  async function kexpKeepKitId(bin) {
    const q = String(location.search || "");
    if (/[?&]keepkitid=0/i.test(q)) { flushMark("KITID", "disabled-by-query"); return false; }
    let tid = -1;
    try {
      const a = kitIdAddr();
      if (!a) return false;
      const r = await kread64Fast(a);
      if (r && r.ret === 8) tid = r.v.low & 0xff;
    } catch (e) {
      flushMark("KITID", "read-failed-" + String((e && e.message) || e).slice(0, 40));
      return false;
    }
    if (tid !== 0x81) {
      flushMark("KITID", "live target_id=" + (tid < 0 ? "<unread>" : "0x" + tid.toString(16))
        + " -> STOCK (kexp forces 0x82); patch is kit-only");
      return false;
    }
    const STORE = [0xc6, 0x45, 0xd6, 0x82];
    const LOG   = [0xba, 0x82, 0x00, 0x00, 0x00];
    for (let k = 0; k < STORE.length; k++)
      if (bin[0x3807 + k] !== STORE[k]) {
        flushMark("KITID", "blob 0x3807 is not the stock target_id store - refusing");
        return false;
      }
    for (let k = 0; k < LOG.length; k++)
      if (bin[0x382a + k] !== LOG[k]) {
        flushMark("KITID", "blob 0x382a is not the stock target_id log - refusing");
        return false;
      }
    for (let k = 0; k < 4; k++) bin[0x3807 + k] = 0x90;
    bin[0x382a] = 0x0f; bin[0x382b] = 0xb6;
    bin[0x382c] = 0x55; bin[0x382d] = 0xd6; bin[0x382e] = 0x90;
    flushMark("KITID", "live target_id=0x81 is a KIT -> blob patched (0x3807 NOPed);"
      + " kexp will leave target_id alone");
    return true;
  }

  async function kitIdSnapshot() {
    try {
      const a = kitIdAddr();
      if (!a) return null;
      const r = await kread64Fast(a);
      if (!r || r.ret !== 8) { flushMark("KITID", "snapshot-unreadable"); return null; }
      const val = r.v.low & 0xff;
      flushMark("KITID", "snapshot target_id=0x" + val.toString(16) + "-at=" + hx(a));
      return { addr: a, val: val };
    } catch (e) {
      flushMark("KITID", "snapshot-failed-" + String((e && e.message) || e).slice(0, 50));
      return null;
    }
  }

  async function kitIdRestore(snap) {
    if (!snap) return;
    try {
      const now = await kread64Fast(snap.addr);
      if (!now || now.ret !== 8) { flushMark("KITID", "restore-unreadable"); return; }
      const cur = now.v.low & 0xff;
      if (cur === snap.val) { flushMark("KITID", "unchanged=0x" + cur.toString(16)); return; }
      /* Write back only the single byte; the neighbouring qaf fields kexp set are
         deliberately left alone - they are what the jailbreak needs. */
      const b = alloc(1, "kitid-byte");
      b.u8[0] = snap.val;
      await kwriteFast(snap.addr, b, 1);
      const back = await kread64Fast(snap.addr);
      const got = (back && back.ret === 8) ? (back.v.low & 0xff) : -1;
      flushMark("KITID",
        "restored 0x" + cur.toString(16) + "->0x" + snap.val.toString(16) +
        "-readback=0x" + (got < 0 ? "??" : got.toString(16)) +
        "-ok=" + (got === snap.val));
    } catch (e) {
      flushMark("KITID", "restore-failed-" + String((e && e.message) || e).slice(0, 50));
    }
  }

  try {
    window.__kitId = { snapshot: kitIdSnapshot, restore: kitIdRestore };
  } catch (e) { }
  ...
  async function stage5Body(opts) {
    ...
    /* Patch the blob BEFORE it is staged or run. Nothing else writes target_id on this
       path: main.js's qaf block belongs to a different main() (the umtx2 one) that
       poops.html never calls, which is why kexp still reports "before qaf target_id 0x81". */
    try { await kexpKeepKitId(binBytes); } catch (e) {
      flushMark("KITID", "patch-threw-" + String((e && e.message) || e).slice(0, 40));
    }
  ...
  /* DIAGNOSTIC. Every ELF ever delivered went through the server-side
     api/payload/ helper - PAYLOAD-DIRECT has never once been emitted - even on runs
     that completed the jailbreak in this very page, where this assignment must have
     run. api/payload/ does not exist on a static host, so if the export is not
     landing, the tiles cannot work on GitHub Pages at all. Report what actually
     happens here rather than reasoning about it. */
  ...
  try {
    window.__sendElfDirect = sendElfDirect;
    flushMark("ELFDIRECT-EXPORT",
      "assigned typeof=" + (typeof window.__sendElfDirect)
      + "-sameWindow=" + (typeof window !== "undefined" && window === globalThis));
  } catch (e) {
    try { flushMark("ELFDIRECT-EXPORT", "FAILED-" + String((e && e.message) || e).slice(0, 60)); } catch (_) { }
  }
