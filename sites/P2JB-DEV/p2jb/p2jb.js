    ...
    try {
        ...
        (function planCores() {
            ...
            if (!Array.isArray(a) || a.length < 3) {
                // DIAGNOSTIC (2026-08-25): this early-out keeps MAIN_CORE=4 and
                // LEAK_CORES=[0,1,2,3] - cores that need not be in this process's allowed
                // mask at all. Pinning RT threads onto cores we are not allowed to use is a
                // plausible route to the spin-lock starvation panic seen on the 12.70 kit,
                // so report it instead of failing silently.
                try {
                    window.syncMark("CORE-PLAN", "allowed=" + JSON.stringify(a || null)
                        + " TOO FEW (<3) -- keeping DEFAULTS leak=[0,1,2,3] exec=4;"
                        + " these may be outside the allowed mask");
                } catch (e) { }
                return;
            }
            ...
            try {
                window.syncMark("CORE-PLAN", "allowed=[" + a.join(",") + "]"
                    ...
                    + " mainReserved=" + a[a.length - 2] + " n=" + a.length);
            } catch (e) { }
        })();
        ...
        const FW_ALIAS_P2JB = {
            ...
            // verified byte-for-byte against the decrypted DEVKIT kernels in
            // I:\extracted on 2026-08-31 (and against I:\americana for the
            // retail site), identical VAs and identical reference counts in
            // every build. It only means a FUTURE 12.x with different offsets
            // is refused loudly instead of silently running 12.00's numbers and
            // panicking.
        };
        ...
        async function stage_debug_menu(S) {
            ...
            try {
                ...
                if (!cr3_ok || !dmap_ok) {
                    ...
                    // verified correct for every 12.x, devkit and retail, by
