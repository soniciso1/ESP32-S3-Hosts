# Flash POOPS (unified poopsploit 7.00-12.00 + p2jb 12.02-12.70 + umtx2) to a
# 16 MB ESP32-S3. One merged image at 0x0. RUN AS ADMIN not required.
#
...
Write-Host "Flashing POOPS-merged.bin to $Port ..." -ForegroundColor Cyan
python -m esptool --chip esp32s3 -p $Port -b 460800 --before default_reset --after hard_reset `
    write_flash --flash_mode dio --flash_freq 80m --flash_size 16MB `
    0x0 "$PSScriptRoot\POOPS-merged.bin"
Write-Host ''
...
Write-Host '"PS5-Jailbreak" on the PS5 and open Manuals.' -ForegroundColor Yellow
