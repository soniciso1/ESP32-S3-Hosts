...
function fireSync(payload, big, spins) {
    ...
    // INSTRUMENTATION 2026-08-29, DEVKIT ONLY. A NEVER-CONSUMED wedge cannot tell
    // "the slave thread never ran" from "it ran but never unwound through our hijacked
    // slot" - and the echo that distinguishes them was being thrown away here. Counting
    // it is side-effect-free: the handler still does nothing else, and nothing reads
    // these fields on the success path.
    W.echoes = 0;
    W.echoAt = 0;
    W.worker.onmessage = function () {
        W.echoes++;
        if (!W.echoAt) W.echoAt = Date.now();
    };
    ...
    /* The spin loop never returned to the event loop, so a queued onmessage CANNOT have
     * dispatched yet - the echoes= above is necessarily 0 and proves nothing on its own.
     * Re-report once the loop drains. echoes>0 there = the slave DID wake and simply did
     * not unwind through our slot (hijack/frame problem). echoes==0 = the worker thread
     * was never scheduled at all (starvation). That is the whole question, and it costs
     * the synchronous path nothing. The page stays up on an unsafe abort, so it lands. */
    try {
        const wedgeT = Date.now();
        setTimeout(function () {
            try {
                const n = W.echoes || 0;
                window.syncMark("RW-WEDGE-ECHO",
                    "echoes=" + n
                    + " echoDelayMs=" + (W.echoAt ? (W.echoAt - wedgeT) : -1)
                    + " fired=" + W.fired
                    + " -> " + (n > 0
                        ? "slave DID wake - it ran but never unwound through our slot"
                        : "slave NEVER woke - the worker thread was not scheduled"));
            } catch (e) { }
        }, 0);
    } catch (e) { }
